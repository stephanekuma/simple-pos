<div class="mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-2">
        @foreach ($products as $product)
            <div wire:click="addToCart({{ $product->id }})" role="button" tabindex="0"
                class="relative bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500">

                <div wire:loading wire:target="addToCart({{ $product->id }})"
                    class="bg-gray-200 dark:bg-gray-900 bg-opacity-80 absolute p-2 w-full h-full text-red-500 z-10">
